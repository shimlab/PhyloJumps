...
from .evals import prior_expected, bayes_factor
...
def pltRest(rest):
    means = rest['clusters']
    axes = sns.kdeplot(rest['data'])
    observations = len(rest['data'])

    # plot cluster means as vertical lines on kdeplot
    clusters = list(means.keys())
    obs_per_cluster = np.array(list(means.values()))
    # scale depending on number of observations belonging to cluster
    heights = obs_per_cluster / observations
    for i in range(len(clusters)):
        plt.axvline(x=clusters[i], ymax=heights[i], color='red')
...
def empirical_dist_one_particle(particle, cmap="autumn", density=True, kde=True, hist=True,
                                fill = False):
    clusters = len(particle)
    colors = [to_hex(c) for c in rgb_scale(clusters, CMAP = cmap, light_bg = True)]
    fig = plt.figure(figsize=(15, 10))
    for i, data in enumerate(particle.values()):
        if hist:
            plt.hist(data, color=colors[i], alpha=0.5, density=density)
        if kde:
            sns.kdeplot(data, color=colors[i], fill = fill, alpha = 0.5)


    return fig

def plot_wishart_mixture(x, params, cmap = "autumn"):
    clusters = params.shape[1]
    if params.shape[0] == 5:
        tree_clusters = len(np.unique(params[4]))
        colors = [to_hex(c) for c in rgb_scale(tree_clusters + 1, CMAP = cmap)]
    else:
        colors = [to_hex(c) for c in rgb_scale(clusters, CMAP = cmap, light_bg = True)]
    # marker to label clusters for legend
    labelled = [0]*clusters
    fig = plt.figure(figsize=(15, 10))
    for i in range(clusters):
        y = params[3][i] * stats.t(params[0][i], params[1][i], params[2][i]).pdf(x)
        if params.shape[0] == 5:
            cluster_index = int(params[4][i])
            if not labelled[cluster_index]:
                plt.plot(x, y, color=colors[cluster_index], label = f"cluster{cluster_index}")
                labelled[cluster_index] =1
            else:
                plt.plot(x,y, color = colors[cluster_index])
        else:
            plt.plot(x,y, color = colors[i])

    return fig

def plot_gamma_mixture(x, params, cmap = "autumn"):
    clusters = params.shape[1]
    if params.shape[0] == 4:
        tree_clusters = len(np.unique(params[3]))
        colors = [to_hex(c) for c in rgb_scale(tree_clusters + 1, CMAP = cmap)]
    else:
        colors = [to_hex(c) for c in rgb_scale(clusters, CMAP = cmap, light_bg = True)]
    fig = plt.figure(figsize=(15, 10))
    ...
    for i in range(clusters):
        ...
        mean = params[0][i]
        sd = params[1][i]
        y = p * stats.norm(mean, sd).pdf(x)
        if params.shape[0] == 4:
            cluster_index = int(params[3][i])
            if not labelled[cluster_index]:
                plt.plot(x, y, color=colors[cluster_index], label=f"cluster{cluster_index}")
                labelled[cluster_index] =1
            else:
...
def plot_wishart_trace(x, all_params, cmap = "autumn", alpha = 0.05, file = None, data = None):
    fig = plt.figure(figsize=(15, 10))
    y = []
    for params in all_params:
        ...
        colors = [to_hex(c) for c in rgb_scale(clusters, CMAP=cmap, light_bg=True)]
        for i in range(clusters):
            y1 = params[3][i] * stats.t(params[0][i], params[1][i], params[2][i]).pdf(x)
            plt.plot(x, y1, color=colors[i], alpha = alpha)
            y.append(y1)

    y = np.array(y).sum(axis = 0) / len(y)
    plt.plot(x, y, color='black', linewidth = 2, label = 'average')
    if data is not None:
        for i in data.cluster.unique():
            sns.kdeplot(data[data['cluster'] == i].obs, linewidth = 2, label = f'real data jump{i}')
    ...
    return fig
def plot_gamma_trace(x, all_params, cmap="autumn",  alpha = 0.05, file = None, data = None):
    fig = plt.figure(figsize=(15, 10))
    y = []
    for params in all_params:
        clusters = params.shape[1]
        colors = [to_hex(c) for c in rgb_scale(clusters, CMAP=cmap, light_bg=True)]
        for i in range(clusters):
            y1 = params[2][i] * stats.norm(params[0][i], params[1][i]).pdf(x)
            plt.plot(x, y1, color=colors[i], alpha=alpha)
            y.append(y1)
...
def plot_predicted_dist(tree, data, predicted_jp, cmap = "autumn", figsize = (8, 12), bins = None, density = True, kde = True, hist = True):
    t = tree.deep_copy()
    t.jps = predicted_jp
    partition = t.partition_by_jps()[0]
    df = pd.DataFrame(data=partition, index=list(tree.nodes.keys())).rename(columns={0: 'predicted_cluster'})
    df = df[df.index.isin(tree.leaf_names)]
    data = data.join(df, on='node_name')
    colors = [to_hex(c) for c in rgb_scale(len(df.predicted_cluster.unique()), CMAP=cmap, light_bg=True)]
    n_clusters = len(df.predicted_cluster.unique())
    fig, axes = plt.subplots(nrows=n_clusters, ncols=1, figsize=figsize, sharey=True, sharex=True)
    fig.subplots_adjust(hspace = 0)
    axes = axes.flatten()
    for i in df.predicted_cluster.unique():
        data_to_plot = data[data.predicted_cluster == i]['obs']
        if i == 0:
            # singleton in cluster
            if data_to_plot.shape[0] == 1:
                axes[i].axvline(x = data_to_plot.iloc[0])
            if hist:
                axes[i].hist(data[data.predicted_cluster == i]['obs'], bins=bins, alpha=0.5, density=density, color = 'steelblue', label = f"Root Distribution")
            if kde:
                sns.kdeplot(data[data.predicted_cluster == i]['obs'], color = 'steelblue', ax = axes[i])

        else:
            if data_to_plot.shape[0] == 1:
                axes[i].axvline(x = data_to_plot.iloc[0], color = colors[i - 1], label = f"Jump {i} Distribution")
            if hist:
                axes[i].hist(data[data.predicted_cluster == i]['obs'], bins=bins, color=colors[i - 1], alpha=0.5, density=density, label=f"jump{i}")
            if kde:
                sns.kdeplot(data[data.predicted_cluster == i]['obs'], color=colors[i - 1], ax = axes[i])

        axes[i].legend()

    fig.suptitle("Data Distribution after Jump Clustering")
    fig.tight_layout()
    return fig, axes
...
def annotated_tree(tree: RestFranchise, **kwargs):
    tree.plot_annotated_tree(**kwargs)
