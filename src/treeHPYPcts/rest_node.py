...
class RestNode(Tree):
    ...
    def __init__(self, newick=None, newick_format=1, dist=0, support=None, name=None,
        ...
        self._depth = 0
        if self.is_root():
            for i,n in enumerate(self.traverse(strategy='preorder')):
                n.id = i
                if n.is_root():
                    n.depth = 0
                else:
    ...
    depth = property(fget=_get_depth, fset =_set_depth)
    ...
    depths = property(fget=_get_depths) # depth of each node
    ...
    def all_saved_subtree_properties(self):
        ...
        for child in self.children:
            tl1, nb1, bls1, jps1 = child.all_saved_subtree_properties() # some recursion going on
            tl += tl1
            nb += nb1
            bls += bls1
            jps += jps1
    ...
    def init_rests(self, disc, conc, base, var):
        """
        ...
        if self.njump == 0:
            self._rest = self.up.rest # pruning part?
        else:
            base = HpypBase(base = base, is_root = self.is_root())
            self._rest = Rest((disc ** self.njump), conc, base, var)  # branch discount = disc^(#jumps)

        for child in self.children:
            child.init_rests(disc, conc, base, var)
    ...
    def update_child_base(self):
        ...
        :return: None
        """
        children_base = self.rest.p_tbl()
        for child in self.children:
            if child.njump > 0:
                child.rest.base.discrete = children_base
            child.update_child_base()

    def find_table(self, table=None, depend=None):
        """
        Update all restaurants with the table configuration introduced by an
        table at current node (self). If table is None, the table configuration
        is sampled according to the prior and the sampled table is returned. If table
        is given, the table configuration then will be sampled according to (6.2)
        in progress report of PhyloTreePY @ 03/28/18.
        :param table: the new observation label, or None.
        :param depend: Boolean, indicating whether the distribution on the child
                       node depends on the parent node.
        :return: sampled label, new_table label
        """
        if depend is None:
        ...
        while (not node.is_root()) and node.njump == 0:
            node = node.up # pruning part, get to the node that has actual jumps

        new_table, k = node.rest.find_table(table)  # if obs is None, then generates an obs from the prior

        if depend:
            if new_table and (not node.is_root()):
                parent = node.up
                parent.find_table(table=k, depend=depend)
            else:
                node.update_child_base()

        return k, new_table

    def seat_obs(self, depend):
        """
        Get an observation from the mixture distribution associated at the node
        """
        # sample a cluster mean
        k, new_table = self.find_table(depend = depend)
        # get mixture observation
        obs = self.rest.seat_customer(table = k)
        return obs, k
